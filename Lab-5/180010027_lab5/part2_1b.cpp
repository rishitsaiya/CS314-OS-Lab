...
void RBG_2_Gray_Conversion(vector<vector<pixel>> &arrayOfData, int height, int width){ 
    ...
    // By considering weights for grayscale, appropriate weighted means are taken
            ...
            sem_post(&binarySemaphore); // Semaphore unlocks
        }
    }
}
    ...
    int pixelWorked = 0;
        ...
        for(int i = 1; i <= height - 2; i++){
            for(int j = 1; j <= width - 2; j++){
                ...
                sem_wait(&binarySemaphore); //waits till the value of semaphore is 1, sets it 0 and breaks and enters critical section
...
int main(int argc, char** argv){
    ...
    sem_init(&binarySemaphore, 0, 1); //second arg 0 means semaphore is shared b/w threads of process, 3rd is init value
